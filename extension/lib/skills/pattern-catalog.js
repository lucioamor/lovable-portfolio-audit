...
export const FALLBACK_BUILTIN = Object.freeze([
  ...
  { id: 'secret_anthropic',    kind: 'secret', severity: 'critical',     regex: 'sk-ant-(?:api03-)?[A-Za-z0-9_-]{20,}', flags: 'g', label: 'Anthropic API Key', enabled: true },
  ...
  { id: 'secret_pem',          kind: 'secret', severity: 'catastrophic', regex: '-----BEGIN (?:RSA|EC|OPENSSH|DSA)? ?PRIVATE KEY-----', flags: 'g', label: 'PEM Private Key', enabled: true },
  { id: 'secret_generic_pwd',  kind: 'secret', severity: 'high',         regex: '(?:password|passwd|pwd|secret)\\s*[:=]\\s*["\']([^"\']{8,})["\']', flags: 'gi', label: 'Generic Password', enabled: true },
  // PII
...
function buildFallback() {
  return Object.freeze({ version: 'builtin-1.2', fetchedAt: new Date().toISOString(), patterns: FALLBACK_BUILTIN });
}
