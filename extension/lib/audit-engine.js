...
// ---- Probe resilience (A7): retry transient errors with linear backoff ----
const PROBE_MAX_ATTEMPTS = 3;
const PROBE_BACKOFF_MS = 334; // ≥334ms per roadmap E6 inter-request delay

async function probeWithRetry(doProbe, ctx) {
  let result;
  ...
  return Object.freeze({ ...result, attempt_number: PROBE_MAX_ATTEMPTS });
}
...
async function scanProject(project, config, ownerToken, auditToken, scanDelay) {
  ...
  for (const ep of PROBE_ENDPOINTS) {
    ...
    if (auditToken && auditToken !== ownerToken) {
      probeResult = await probeWithRetry(() => probeDual(endpoint, project.id, probeConfig), { endpoint, projectId: project.id, label: ep.label });
    } else {
