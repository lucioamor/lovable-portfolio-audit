...
import { scanBundle, type BundleScanResult } from '../engines/bundle-scan.js';
import {
  calculateRiskScore,
...
async function scanProject(
  ...
  // Feed Supabase creds discovered in the deployed bundle into the RLS probe.
  if (bundle) {
    if (!config.supabaseUrl && bundle.supabaseUrl) config.supabaseUrl = bundle.supabaseUrl;
    if (!config.supabaseAnonKey && bundle.supabaseAnonKeyRaw) config.supabaseAnonKey = bundle.supabaseAnonKeyRaw;
  }
  ...
  // ---- Bundle / runtime findings (B1/B2) ----
  if (bundle?.reachable) {
    // B1: inline source map → source is exposed even with no .map served.
    if (bundle.sourceMapInline) {
      findings.push(makeFinding(
        'P8', 'source_map_exposed', 'high',
        'Inline Source Map in Production Bundle',
        'A chunk embeds an inline data: source map (//# sourceMappingURL=data:...), exposing original source even though no separate .map file is served.',
        `Inline sourceMappingURL detected in bundle of ${bundle.url}`,
        'Disable source map generation (or inline maps) in the production build.',
        'In Lovable, ask the AI: "Disable source maps in the production Vite build — set build.sourcemap to false in vite.config.ts so no inline or external maps ship."',
      ));
    }

    // B1: external subresources without SRI.
    for (const sri of bundle.scriptsWithoutSri.slice(0, 10)) {
      findings.push(makeFinding(
        'P8-SRI', 'missing_sri', 'medium',
        `External <${sri.tag}> without Subresource Integrity`,
        'A cross-origin script/style is loaded without an integrity attribute. If the CDN is compromised, altered code runs with your app\'s privileges.',
        `${sri.tag} ${sri.url} — no integrity=`,
        'Add an integrity (SRI) hash and crossorigin attribute, or self-host the resource.',
        `In Lovable, ask the AI: "Add Subresource Integrity (integrity + crossorigin) to the external ${sri.tag} loading ${sri.url}, or self-host it."`,
      ));
    }

    // B2: Supabase anon/publishable key shipped in the bundle.
    if (bundle.supabaseAnonKeyRaw) {
      findings.push(makeFinding(
        'P9', bundle.supabaseKeyIsServiceRole ? 'service_role_exposed' : 'anon_key_irrestrito',
        bundle.supabaseKeyIsServiceRole ? 'catastrophic' : 'medium',
        bundle.supabaseKeyIsServiceRole
          ? 'CATASTROPHIC: service_role Key in Production Bundle'
          : 'Supabase URL + anon key embedded in bundle',
        bundle.supabaseKeyIsServiceRole
          ? 'The key shipped in the bundle decodes to role=service_role, bypassing all RLS.'
          : 'The deployed bundle contains the Supabase project URL and anon/publishable key. This is public by design, but only RLS protects the data behind it.',
        `${bundle.supabaseUrl || 'supabase'} · key ${bundle.supabaseAnonKeyMasked}`,
        bundle.supabaseKeyIsServiceRole
          ? 'Rotate the key immediately and move privileged calls server-side.'
          : 'Confirm RLS is enabled and forced on every table (run the deep RLS checklist).',
        'In Lovable, ask the AI: "Verify Row Level Security is enabled on all Supabase tables, since the anon key is public in the deployed bundle."',
      ));
    }
  }

  // B2 compound risk: public anon key + an RLS gap on the same project is worse
  // than either alone — an attacker with the (public) key can read/write exposed
  // tables directly. Sum both signals into one critical finding.
  const anonKeyExposed = !!(bundle?.supabaseAnonKeyRaw) || !!config.supabaseAnonKey;
  const rlsGap = findings.find(f =>
    f.vector === 'rls_missing' || f.vector === 'rls_permissive' ||
    ['DB-002', 'DB-003', 'DB-005', 'DB-008', 'LOV-002'].includes(f.ruleId));
  if (anonKeyExposed && rlsGap) {
    findings.push(makeFinding(
      'WS-3', 'compound_risk', 'critical',
      'Compound Risk: Public anon key + RLS gap',
      'The Supabase anon/publishable key is exposed publicly AND at least one table lacks effective RLS. Together these let anyone with the (public) key read or modify the exposed data directly — no BOLA needed.',
      `anon key exposed + ${rlsGap.ruleId} (${rlsGap.title})`,
      'Close the RLS gap first; the public key is only safe when every table enforces (and forces) RLS.',
      'In Lovable, ask the AI: "Enable and FORCE Row Level Security with owner-scoped policies on every table, because the anon key is publicly exposed in the bundle."',
    ));
  }
  ...
  return {
    ...
    rlsStatus: 'unknown',
    sourceMapsExposed: !!(bundle?.sourceMapInline || (bundle?.sourceMapExternalRefs.length ?? 0) > 0),
  };
}
...
export async function runScan(
  ...
  // Bundle/runtime scan runs once against the provided deployed URL (B1/B2).
  let bundle: BundleScanResult | undefined;
  if (config.targetUrl && config.testSourceMaps) {
    onProgress?.('🔎 Analyzing deployed bundle (source maps, SRI, Supabase keys)...');
    bundle = await scanBundle(config.targetUrl);
  }
  ...
  for (let i = 0; i < projects.length; i++) {
    ...
    try {
      const result = await scanProject(project, client, { ...config }, onProgress, bundle);
      results.push(result);
    } catch (err) {
